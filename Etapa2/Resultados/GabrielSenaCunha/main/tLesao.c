...
void adicionaListaLesao(tListaLesao *lista, tLesao *lesao){
    ...
    if(retornaSeLesaoParaCirurgia(lesao))
        lista->paraCirurgia++;
...
void desalocaListaLesao(tListaLesao *lista){
    if(lista != NULL){
        for(int i = 0; i < retornaNumeroLesoes(lista); i++){
...
void PrintaListaLesao(tListaLesao *lista, FILE *arq){
    for(int i = 0; i < lista->qtd; i++){
        fprintf(arq, "- -\n");
        fprintf(arq, "ROTULO: %s\n", lista->lesao[i]->rotulo);
        fprintf(arq, "DIAGNOSTICO CLINICO: %s\n", lista->lesao[i]->diagnostico);
        fprintf(arq, "REGIAO DO CORPO: %s\n", lista->lesao[i]->regiao);
        fprintf(arq, "TAMANHO: %d\n", lista->lesao[i]->tamanho);
        if(lista->lesao[i]->cirurgia)
            fprintf(arq, "ENVIADA PARA CIRURGIA: SIM\n");
        else 
            fprintf(arq, "ENVIADA PARA CIRURGIA: NAO\n");

        if(lista->lesao[i]->crioterapia)
            fprintf(arq, "ENVIADA PARA CRIOTERAPIA: SIM\n");
        else
            fprintf(arq, "ENVIADA PARA CRIOTERAPIA: NAO\n");
    }
}