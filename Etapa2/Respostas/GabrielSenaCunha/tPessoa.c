...
void recuperaInicio(tListaPessoas *lista, FILE *arq){
  ...
  fread(pessoa->nome, sizeof(char), 100, arq);
  fread(pessoa->CPF, sizeof(char), 15, arq);
  fread(&pessoa->dia, sizeof(int), 1, arq);
  fread(&pessoa->mes, sizeof(int), 1, arq);
  fread(&pessoa->ano, sizeof(int), 1, arq);
  fread(pessoa->telefone, sizeof(char), 15, arq);
  fread(pessoa->genero2, sizeof(char), 13, arq);
  pessoa->genero = pessoa->genero2[0];
  fread(pessoa->senha, sizeof(char), 20, arq);
  fread(pessoa->usuario, sizeof(char), 20, arq);
  fread(&pessoa->cargo, sizeof(char), 1, arq);

  lista->pessoa[0] = pessoa;
}

void printaMedicos(tListaPessoas *lista, char *path){
  char diretorio[200];
  sprintf(diretorio, "%s/medicos.txt", path);
  FILE *arq = fopen(diretorio, "a");

  for(int i = 0; i < lista->qtd; i++){
    if(lista->pessoa[i]->cargo == 'M'){
      fprintf(arq, "NOME: %s\n", lista->pessoa[i]->nome);
      fprintf(arq, "CPF: %s\n", lista->pessoa[i]->CPF);
      fprintf(arq, "DATA DE NASCIMENTO: %d/%d/%d\n", lista->pessoa[i]->dia, lista->pessoa[i]->mes, lista->pessoa[i]->ano);
      fprintf(arq, "TELEFONE: %s\n", lista->pessoa[i]->telefone);
      fprintf(arq, "GENERO: ");
      if(lista->pessoa[i]->genero == 'M')
        fprintf(arq, "MASCULINO\n");
      if(lista->pessoa[i]->genero == 'F')
        fprintf(arq, "FEMININO\n");
      if(lista->pessoa[i]->genero == 'O')
        fprintf(arq, "OUTROS\n");
      fprintf(arq, "CRM: %s\n", lista->pessoa[i]->CRM);
      fprintf(arq, "NOME DE USUARIO: %s\n", lista->pessoa[i]->usuario);
      fprintf(arq, "- - - - - -\n");
    }
  }
...
void printaSecretarios(tListaPessoas *lista, char *path){
  char diretorio[200];
  sprintf(diretorio, "%s/secretarios.txt", path);
  FILE *arq = fopen(diretorio, "a");

   for(int i = 0; i < lista->qtd; i++){
    if(lista->pessoa[i]->cargo == 'A' || lista->pessoa[i]->cargo == 'U'){
      fprintf(arq, "NOME: %s\n", lista->pessoa[i]->nome);
      fprintf(arq, "CPF: %s\n", lista->pessoa[i]->CPF);
      fprintf(arq, "DATA DE NASCIMENTO: %d/%d/%d\n", lista->pessoa[i]->dia, lista->pessoa[i]->mes, lista->pessoa[i]->ano);
      fprintf(arq, "TELEFONE: %s\n", lista->pessoa[i]->telefone);
      ...
      if(lista->pessoa[i]->genero == 'O')
        fprintf(arq, "OUTROS\n");
      fprintf(arq, "NOME DE USUARIO: %s\n", lista->pessoa[i]->usuario);
      if(lista->pessoa[i]->cargo == 'A')
        fprintf(arq, "NIVEL DE ACESSO: ADMIN\n");
      if(lista->pessoa[i]->cargo == 'U')
        fprintf(arq, "NIVEL DE ACESSO: USER\n");
      fprintf(arq, "- - - - - -\n");
    }
  }
